...
def enrich_skill_meta(meta: dict, slug: str, character: str | None = None) -> dict:
    ...
    engine["kind"] = "meta-skill"
    engine["character"] = resolved_character
    engine["research_profile"] = resolved_research_profile
    engine["preset"] = result["preset"]
    engine["prompt_bundle"] = preset["prompt_bundle"]
    engine["research_profile_bundle"] = research_profile.get("prompt_bundle", {})
    engine["research_profile_references"] = research_profile.get("references", [])
    engine["merge_strategy"] = research_profile.get("merge_strategy", "compact")
    engine["quality_profile"] = research_profile.get("quality_profile", "budget-friendly")
    engine["knowledge_dirs"] = preset.get("knowledge_dirs", [])
    engine["storage_root"] = preset.get("storage_root", preset["legacy_storage_root"])
    if preset.get("research_tools"):
        engine["research_tools"] = preset["research_tools"]
    if preset.get("streamer_tools"):
        engine["streamer_tools"] = preset["streamer_tools"]
    if preset.get("references"):
        engine["references"] = preset["references"]

    generation["engine"] = "dot-skill"
    generation["character"] = resolved_character
    generation["research_profile"] = resolved_research_profile
    generation["preset"] = result["preset"]
    generation["prompt_bundle"] = preset["prompt_bundle"]
    generation["research_profile_bundle"] = research_profile.get("prompt_bundle", {})
    generation["research_profile_references"] = research_profile.get("references", [])
    generation["merge_strategy"] = research_profile.get("merge_strategy", "compact")
    generation["quality_profile"] = research_profile.get("quality_profile", "budget-friendly")
    generation["knowledge_dirs"] = preset.get("knowledge_dirs", [])
    generation["storage_root"] = preset.get("storage_root", preset["legacy_storage_root"])
    if preset.get("research_tools"):
        generation["research_tools"] = preset["research_tools"]
    if preset.get("streamer_tools"):
        generation["streamer_tools"] = preset["streamer_tools"]
    if preset.get("references"):
        generation["references"] = preset["references"]
...
def build_manifest(meta: dict) -> dict:
    ...
    if meta["character"] == "streamer":
        capabilities.extend(["script_generator", "compliance"])
    return {
        "manifest_version": "1",
        "id": meta["id"],
        "kind": meta["kind"],
        ...
        "toolchain": {
            ...
            "streamer_tools": meta["engine"].get("streamer_tools", {}),
            "references": meta["engine"].get("references", []),

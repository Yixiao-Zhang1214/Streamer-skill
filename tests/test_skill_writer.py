...
class SkillWriterTest(unittest.TestCase):
    ...
    def test_create_streamer_adds_generator_and_compliance_toolchain(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            meta = {
                ...
                "name": "Pencil Xiaoxin",
                "profile": {
                    "identity": "Douyin plus-size womenswear live-commerce creator",
                    "specialty": "try-on selling and size recommendation",
                },
                "classification": {"language": "en"},
                "streamer_config": {
                    "main_category": "womenswear",
                    "target_audience": "plus-size women",
                    "price_band": "affordable",
                },
            ...
            self.assertEqual(saved_meta["character"], "streamer")
            self.assertEqual(saved_meta["preset"], "dot.streamer.v1")
            self.assertEqual(saved_meta["type"], "streamer")
            self.assertEqual(saved_meta["classification"]["gallery_category"], "Streamer")
            self.assertEqual(saved_meta["engine"]["storage_root"], "skills/streamer")
            self.assertIn("script_generator", saved_meta["engine"]["prompt_bundle"])
            self.assertIn("compliance", saved_meta["engine"]["prompt_bundle"])
            self.assertIn("streamer_tools", saved_meta["engine"])
            self.assertIn("streamer_compliance_check", saved_meta["engine"]["streamer_tools"])
            self.assertIn("streamer_quality_check", saved_meta["engine"]["streamer_tools"])
            self.assertIn("references/compliance_blacklist.md", saved_meta["engine"]["references"])
            self.assertIn("script_generator", manifest["capabilities"])
            self.assertIn("compliance", manifest["capabilities"])
            self.assertIn("streamer_tools", manifest["toolchain"])
            self.assertEqual(manifest["id"], "meta-skill.streamer.pencil_xiaoxin")
            self.assertEqual(saved_meta["artifacts"]["combined_command"], "streamer-pencil-xiaoxin")
            self.assertIn("name: streamer_pencil_xiaoxin", combined_skill)
            self.assertTrue((skill_dir / "knowledge" / "asr" / "raw").exists())
            self.assertTrue((skill_dir / "knowledge" / "asr" / "segmented").exists())
            self.assertTrue((skill_dir / "knowledge" / "commerce").exists())
            self.assertTrue((skill_dir / "knowledge" / "conversion").exists())

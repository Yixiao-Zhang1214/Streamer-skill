---
name: dot-skill-copy
description: "Unified meta-skill engine for distilling colleague, relationship, celebrity, or streamer characters into reusable Skills. | 统一的 meta-skill 引擎，把 colleague、relationship、celebrity、streamer 四类对象蒸馏成可复用 Skill。"
...
统一主入口是 `dot-skill-copy`。在支持 slash command 的宿主中，使用 `/dot-skill-copy`。
对 Hermes 而言，只保证 `/dot-skill-copy` 这一条 slash 入口稳定；`colleague`、`relationship`、`celebrity`、`streamer` 的兼容语义保留在工具层和 preset 层，但不保证每个兼容名称都能作为 Hermes slash command 被路由。
...
| 抖音达人主页 URL 解析 | `Bash` → `python3 tools/douyin_profile_parser.py` |
| 直播 ASR 阶段分段 | `Bash` → `python3 tools/asr_segmenter.py` |
| 直播 ASR 深度指标 | `Bash` → `python3 tools/streamer_deep_metrics.py` |
| streamer 产物质量检查 | `Bash` → `python3 tools/streamer_quality_check.py` |
| 直播带货话术合规检查 | `Bash` → `python3 tools/streamer_compliance_check.py` |
| 直播带货话术机评检查 | `Bash` → `python3 tools/streamer_script_quality_check.py` |
...
如果用户使用的是 `/dot-skill-copy`，先确认本次要蒸馏的是哪一类：

1. `colleague`
2. `relationship`
3. `celebrity`
4. `streamer`（抖音平台直播带货达人）
...
- `streamer` → `prompts/streamer/intake.md`

`colleague` 和 `relationship` 只问 3 个问题。
`celebrity` 按 `prompts/celebrity/intake.md` 问 4 个问题，其中第 4 个问题必须确认 `research_profile`。
`streamer` 按 `prompts/streamer/intake.md` 做“抖音主页链接优先、缺啥补啥”的渐进式录入，并要求商品类目、目标受众和直播 ASR。
...
| `streamer` | `prompts/streamer/intake.md` | `prompts/streamer/persona_analyzer.md` | `prompts/streamer/persona_builder.md` | `prompts/streamer/merger.md` | `./skills/streamer/{slug}` |

所有 family 共用：
- Work analyzer：默认 `prompts/work_analyzer.md`；`streamer` 使用 `prompts/streamer/work_analyzer.md`
- Work builder：默认 `prompts/work_builder.md`；`streamer` 使用 `prompts/streamer/work_builder.md`
- Correction handler：`prompts/correction_handler.md`

如果当前是 `celebrity`，必须先走 research 子流程，再进入分析。
如果当前是 `streamer`，必须读取 `prompts/streamer/script_generator.md`、`prompts/streamer/compliance.md`、`references/live_script_framework.md`、`references/script_evaluation_rubric.md` 和 `references/compliance_blacklist.md`，把“深度话术模型 + 话术生成器 + 机评标准 + 合规层”写入最终 Skill。`streamer` 产物必须对标或超过 `livestream-host-zhouzhou` 的深度，不能只生成简版 7 层人格摘要。

### streamer / deep live-commerce profile

1. 先解析主页公开信息：
   ```bash
   python3 tools/douyin_profile_parser.py "{douyin_profile_url}" --pretty
   ```
   - 只使用普通公开读取：无登录、无浏览器自动化、无反爬绕过。
   - 记录昵称、抖音号、UID、sec_uid、简介、作品数、粉丝展示口径和字段来源。
2. 如果用户提供 ASR / transcript，先做阶段分段：
   ```bash
   python3 tools/asr_segmenter.py "{asr_txt}" --output "{skill_dir}/knowledge/asr/segmented/segments.json" --pretty
   ```
3. 同时必须计算深度指标：
   ```bash
   python3 tools/streamer_deep_metrics.py "{asr_txt}" --output "{skill_dir}/knowledge/asr/segmented/deep_metrics.json" --pretty
   ```
   至少提取：句长、问号数、感叹号数、第一人称、称呼、价格锚点、赠品信号、稀缺信号、转化动作、决策简化信号。
4. 必须创建 research 笔记，不允许只写最终 `persona.md` / `work.md`：
   - `knowledge/research/raw/01_public_profile_and_scope.md`
   - `knowledge/research/raw/02_asr_expression_metrics.md`
   - `knowledge/research/raw/03_conversion_methodology.md`
   - `knowledge/research/merged/summary.md`
5. `persona.md` 必须是深度公开直播间人格模型，包含：
   - 数据底座
   ...
   - Commercial Persona
   - Anti-Features / 不像她的表达
   - Evidence Boundaries
6. `work.md` 必须是可操作方法论系统，包含：
   - Input Contract
   - 5-8 个核心方法论
   - 带货 7 阶段 Selling Flow
   - 10+ 个可替换话术模板
   - 5 分钟提词器或 60/90 分钟直播结构
   - 商品诊断规则
   - Script Generator
         - Script Evaluation Rubric / 机评标准（P0 硬门槛 + P1 增强项）
   - 直播前自检清单
   - 品类迁移矩阵
   - Compliance Gate
   - Anti-Features
7. `meta.json` 必须写入 `key_metrics` 和 `signature_phrases`，使产物可审计。
8. 生成后必须运行：
   ```bash
   python3 tools/streamer_quality_check.py "{skill_dir}"
   ```
   如果未通过，先补 `work.md` / `persona.md` / `knowledge/research`，不要把简版产物交付给用户。
9. 生成直播话术时，必须按 `references/script_evaluation_rubric.md` 做机评自检，并在输出给用户前运行：
   ```bash
   python3 tools/streamer_script_quality_check.py "{script_file}"
   python3 tools/streamer_compliance_check.py "{script_file}"
   ```
   如果机评出现 P0 失败，或合规检查出现 `BLOCK`，必须先改写；如果只有 P1 失败，应优先增强，除非用户明确要求极短话术。
...
- streamer 场景下，`work` 必须包含深度方法论、带货 7 阶段、输入契约、模板库、直播结构、话术生成器规则、品类迁移、反特征和合规关卡
...
使用当前 family 对应的 Work builder 生成 Work 内容；`streamer` 使用 `prompts/streamer/work_builder.md`。
...
- `streamer` → `prompts/streamer/persona_builder.md`
...
3. `meta.json` 至少包含：
   ...
   - `display_name`
   - `character`
   - `research_profile`（当 character=`celebrity` 时必填；`streamer` 可省略或使用 `standard`）
   - `classification.language`（必须设置为用户当前语言，例如 `zh-CN` 或 `en`）
   - `profile`
   - `tags`
   - `knowledge_sources`
   - `streamer_config`（当 character=`streamer` 时建议包含主营类目、价格带、目标受众、合规配置）
4. 然后调用：
   ```bash
...
6. 如果当前是 `celebrity`，创建完成后必须再跑一次质量检查：
   ...
   ```
7. 如果当前是 `streamer`，创建完成后必须再跑一次 streamer 质量检查：
   ```bash
   python3 tools/streamer_quality_check.py "{resolved_base_dir}/{slug}"
   ```
   如果该检查失败，先补齐 `script_generator`、`compliance`、输入契约、带货流程、观众关系或证据边界，不要跳过。
8. 如果生成直播带货成稿，输出给用户前必须把成稿写入临时文件并执行合规检查：
   ```bash
   python3 tools/streamer_compliance_check.py /tmp/streamer_script_{slug}.md
   ```
   若出现 `BLOCK`，必须改写或移除风险话术；若出现 `CONFIRM`，必须标注需用户确认的事实。
9. 如果 `celebrity` 的质量检查仍然提示 `source_grounding` 失败：
   - 可以补写诚实的来源说明和局限说明
...
python3 tools/skill_writer.py --action list --character streamer --base-dir ./skills/streamer
```
...
# streamer
python3 tools/version_manager.py --action rollback --character streamer --slug {slug} --version {version} --base-dir ./skills/streamer
```
...
# dot-skill-copy Creator (Compatible Host Edition)
...
The canonical entrypoint is `dot-skill-copy`. In hosts that expose slash commands, use `/dot-skill-copy`.
Under Hermes specifically, only `/dot-skill-copy` is guaranteed as a stable slash entrypoint. Compatibility semantics for `colleague`, `relationship`, `celebrity`, and `streamer` remain in the tool layer and preset layer, but Hermes does not guarantee that every compatibility name will be routed as a slash command.
...
| Parse Douyin streamer profile URL | `Bash` → `python3 tools/douyin_profile_parser.py` |
| Segment live ASR by stage | `Bash` → `python3 tools/asr_segmenter.py` |
| Compute deep live ASR metrics | `Bash` → `python3 tools/streamer_deep_metrics.py` |
| Check streamer artifact quality | `Bash` → `python3 tools/streamer_quality_check.py` |
| Check live-commerce script compliance | `Bash` → `python3 tools/streamer_compliance_check.py` |
| Check live-commerce script rubric quality | `Bash` → `python3 tools/streamer_script_quality_check.py` |
...
If the user entered `/dot-skill-copy`, first confirm which family should be distilled:

1. `colleague`
2. `relationship`
3. `celebrity`
4. `streamer` (Douyin live-commerce creator)
...
For `streamer`, use `prompts/streamer/intake.md`: start from the Douyin profile URL when present, then fill missing category, target audience, and live ASR fields.
...
- Work analyzer: default `prompts/work_analyzer.md`; `streamer` uses `prompts/streamer/work_analyzer.md`
- Work builder: default `prompts/work_builder.md`; `streamer` uses `prompts/streamer/work_builder.md`
- Correction handler: `prompts/correction_handler.md`

If the current family is `celebrity`, run the research subflow before analysis.
If the current family is `streamer`, read `prompts/streamer/script_generator.md`, `prompts/streamer/compliance.md`, `references/live_script_framework.md`, `references/script_evaluation_rubric.md`, and `references/compliance_blacklist.md`, then embed a deep live-commerce method model, script generator, script evaluation rubric, and compliance layer into the generated Skill. `streamer` output must match or exceed the depth of the `livestream-host-zhouzhou` reference; do not produce a thin seven-layer persona summary.

### streamer / deep live-commerce profile

1. Parse the public Douyin profile first:
   ```bash
   python3 tools/douyin_profile_parser.py "{douyin_profile_url}" --pretty
   ```
   Use ordinary public HTTP only: no login, no browser automation, and no anti-bot bypass. If the public page is limited or protected, record the limitation instead of inventing profile facts.
2. Segment live ASR before drafting:
   ```bash
   python3 tools/asr_segmenter.py "{asr_txt}" --output "{skill_dir}/knowledge/asr/segmented/stage_segments.json" --pretty
   ```
3. Compute deep live-commerce ASR metrics:
   ```bash
   python3 tools/streamer_deep_metrics.py "{asr_txt}" --output "{skill_dir}/knowledge/asr/segmented/deep_metrics.json" --pretty
   ```
   Extract at minimum: sentence length, question marks, exclamation marks, first-person count, address terms, price anchors, gift signals, scarcity signals, conversion actions, and decision-simplification signals.
4. Create research notes. Never write only final `persona.md` / `work.md`:
   - `knowledge/research/raw/01_public_profile_and_scope.md`
   - `knowledge/research/raw/02_asr_expression_metrics.md`
   - `knowledge/research/raw/03_conversion_methodology.md`
   - `knowledge/research/merged/summary.md`
5. `persona.md` must be a deep public live-room persona model with:
   - data foundation
   - Core Performance Rules
   - Streamer Context
   - persona profile
   ...
   - 5-8 thinking / conversion models
   - Emotional Curve
   - Audience Relationship
   - Commercial Persona
   - Anti-Features / unlike-this-streamer expressions
   - Evidence Boundaries
6. `work.md` must be an operational method system with:
   - Input Contract
   - 5-8 core methodology models
   - 7-stage live-commerce Selling Flow
   - 10+ replaceable script templates
   - 5-minute teleprompter loop or 60/90-minute live structure
   - product diagnosis rules
   - Script Generator
   - Script Evaluation Rubric with P0 hard gates and P1 quality enhancers
   - pre-live self-check checklist
   - category migration matrix
   - Compliance Gate
   - Anti-Features
7. `meta.json` must include `key_metrics` and `signature_phrases` so the result is auditable.
8. After generation, run:
   ```bash
   python3 tools/streamer_quality_check.py "{skill_dir}"
   ```
   If the check fails, repair `work.md`, `persona.md`, or `knowledge/research` first; do not hand over a simplified artifact.
9. When generating live-commerce scripts, apply `references/script_evaluation_rubric.md` and run both checks before showing the script to the user:
   ```bash
   python3 tools/streamer_script_quality_check.py "{script_file}"
   python3 tools/streamer_compliance_check.py "{script_file}"
   ```
   If the rubric check has P0 failures or compliance has `BLOCK` findings, revise before delivery. If only P1 fails, improve it unless the user explicitly asks for a very short script.
...
- For `streamer`, `work` must include deep methodology, the 7-stage selling flow, input contract, template bank, live-session structure, script-generator rules, product diagnosis, category migration, anti-features, and compliance gate
...
- For `celebrity`, retain:
  ...
  - honest boundaries
- For `streamer`, retain:
  - data foundation and quantitative metrics
  - live-room expression DNA
  - Signature Phrases
  - thinking / conversion models
  - emotional curve
  - audience relationship
  - commercial persona
  - anti-features
...
Use the family-specific Work builder to generate Work content; `streamer` uses `prompts/streamer/work_builder.md`.
...
3. The temporary meta file must include at least:
   ...
   - `display_name`
   - `character`
   - `research_profile` (required when `character=celebrity`; optional or `standard` for `streamer`)
   - `classification.language` (must match the user's language, for example `zh-CN` or `en`)
   - `profile`
   - `tags`
   - `knowledge_sources`
   - `streamer_config` (recommended when `character=streamer`: category, price band, audience, compliance config)
4. Then call:
   ```bash
   python3 tools/skill_writer.py \
     --action create \
...
6. If the current family is `celebrity`, run a quality check after creation:
   ...
   python3 tools/research/quality_check.py "{resolved_base_dir}/{slug}/SKILL.md" --profile {research_profile}
   ```
7. If the current family is `streamer`, run the streamer quality check after creation:
   ```bash
   python3 tools/streamer_quality_check.py "{resolved_base_dir}/{slug}"
   ```
   If it fails, repair `script_generator`, `compliance`, input contract, selling flow, audience relationship, evidence boundaries, depth, or research artifacts before delivery.
8. If generating a live-commerce final script, write it to a temporary file and run the script rubric checker plus compliance checker before showing it to the user:
   ```bash
   python3 tools/streamer_script_quality_check.py /tmp/streamer_script_{slug}.md
   python3 tools/streamer_compliance_check.py /tmp/streamer_script_{slug}.md
   ```
   If the rubric check has P0 failures, revise the script. If any `BLOCK` finding appears, rewrite or remove the risky wording. If any `CONFIRM` finding appears, mark the underlying fact as requiring user confirmation.
9. If `source_grounding` still fails for a `celebrity` skill:
...
9. If the current family is `streamer`, run the streamer quality check again after the update:
   ```bash
   python3 tools/streamer_quality_check.py "{resolved_base_dir}/{slug}"
   ```
   If the check fails, keep the update unfinished and repair the weak sections before reporting success.
...
6. If the current family is `streamer`, run the streamer quality check again after the update:
   ```bash
   python3 tools/streamer_quality_check.py "{resolved_base_dir}/{slug}"
   ```
   If the check fails, do not claim the correction is complete.
7. Do not hand-edit `work.md`, `persona.md`, `SKILL.md`, or `meta.json`; always update through `skill_writer.py`

---

## Management Operations

List skills across the four families:
```bash
python3 tools/skill_writer.py --action list --character colleague --base-dir ./skills/colleague
python3 tools/skill_writer.py --action list --character relationship --base-dir ./skills/relationship
python3 tools/skill_writer.py --action list --character celebrity --base-dir ./skills/celebrity
